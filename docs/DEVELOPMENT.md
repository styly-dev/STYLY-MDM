...
| `PUSH_JOB_ACCEPTED` / `PUSH_JOB_REJECTED` | Exact command acceptance after the client durably persists active state, or a duplicate-safe rejection (`device_busy`, malformed identity, destination/artifact conflict). Fields include `job_id`, fixed `attempt=1`, and current phase/reason. |
