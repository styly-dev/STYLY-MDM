...
| Per-device durable queue | Conformant | monotonic `enqueue_seq`; globally oldest non-terminal queued row cannot be overtaken; closed dispatch gates remain operator-visible |
| Shared global transfer pool | Conformant | Install, legacy Push, and job-v1 Push use one semaphore and typed registry |
| Waiter-before-send ordering | Conformant | exact waiter exists before `dispatching` commit and WebSocket send; cleanup is future-identity checked |
...
| Command acceptance reconciliation | Conformant | live waiter owns its 15s timeout; exact-token durable sweep recovers waiter loss; 60s exact probe; one same-attempt replay on explicit absence only |
| Long-running reconciliation | Conformant | 1800s default; deadline callback validates exact stored deadline before `unconfirmed`; exact absence clears the matching fence atomically |
...
| Superseded WebSocket guard | Conformant | inbound settlement, outbound send, disconnect transition, and replacement registration share the per-device owner lock |
| Console reload reconstruction | Conformant | full-replacement snapshot, pre-snapshot update buffer, revisioned merge, canonical per-device rows, stable Dispatch/Resume actions, and safe fence reconcile action |
...
44. exact terminal evidence could be rejected when an intermediate phase frame was lost or reconciliation timed out concurrently;
45. completed receipts could not answer a later exact reconciliation request;
46. disconnect could release the owner lock before committing its canonical transition, allowing a replacement REGISTER to be overwritten;
47. exact absence could race the reconciliation deadline and leave a matching fence behind;
48. an uploaded but undispatched ready job had no stable console dispatch action;
49. waiter cleanup was not identity-safe and durable accept deadlines were not swept with an exact dispatch token;
50. direct admin acknowledgement I/O delayed scheduler wake after dispatch was durably enabled;
51. the durable accept sweep could race a live local waiter and release its transfer slot after reconciliation recovered the assignment;
52. a failed admin `ERROR` reply could escape the WebSocket receive loop without the standard failed-socket cleanup.
...
strict JSON types, absolute artifact URLs, archive path conflicts, destination symlink
escape, replacement registration, live-waiter-aware acceptance deadline recovery,
admin reply failure, immediate dispatch wake, and ready-job dispatch.
