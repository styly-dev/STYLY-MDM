...
def require_uuid(value: Any, field: str) -> str:
    if not isinstance(value, str):
        raise PushJobError(f"{field} must be a UUIDv4 string")
    try:
        parsed = uuid.UUID(value)
    except (ValueError, AttributeError) as exc:
        raise PushJobError(f"{field} must be a valid UUIDv4") from exc
    if parsed.version != 4:
        raise PushJobError(f"{field} must be a UUIDv4")
    return str(parsed)
